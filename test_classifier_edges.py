...
def sub_mean_edge_detector(image):
    new_batch = torch.zeros_like(image)
    means = [104.00698793,116.66876762,122.67891434]
    new_batch[:,0] = (image[:,0] * 255.0) - means[0]
    new_batch[:,1] = (image[:,1] * 255.0) - means[1]
    new_batch[:,2] = (image[:,2] * 255.0) - means[2]
    return new_batch


def validate(val_loader, edge_detector, model, criterion, args):

    adversary = PGD(epsilon=8.0/255, num_steps=20, step_size=2.0/255).cuda()
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(tqdm(val_loader)):
            ...
            image_adv = adversary(model, edge_detector, bx.clone(), target.clone())
            ...
            # compute output
            image_adv = edge_detector(sub_mean_edge_detector(image_adv))[-1]
...
class PGD(nn.Module):
    ...
    def forward(self, model, edge_detector, bx, by):
        """
        ...
        for i in range(self.num_steps):
            ...
            with torch.enable_grad():
                output = model(edge_detector(sub_mean_edge_detector(adv_bx))[-1])
